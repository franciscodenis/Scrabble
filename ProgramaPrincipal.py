...
def main():
    ...
    while True:
        ...
        if event == "cambiar_letras":
            atril.cambiar_letras(fichas_jugador,window,tablero,values,fichas_jugador)

        if event in atril.listado_botones():
            atril.click(tablero, event)

        if event == 'vali':
            puntaje_total = tablero.click_validar(atril, tablero, window, diccionario, puntaje_total, fichas_jugador, palabras_permitidas)
